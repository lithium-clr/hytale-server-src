...
public class Short2ObjectMapCodec<T> implements Codec<Short2ObjectMap<T>>, WrappedCodec<T> {
   ...
   public BsonValue encode(@Nonnull Short2ObjectMap<T> map, ExtraInfo extraInfo) {
      ...
      while (var4.hasNext()) {
         it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<T> entry = (it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<T>)var4.next();
         bsonDocument.put(Short.toString(entry.getShortKey()), this.valueCodec.encode((T)entry.getValue(), extraInfo));
      }
