...
public class AssetModule extends JavaPlugin {
   ...
   protected void setup() {
      if (Options.getOptionSet().has(Options.DISABLE_FILE_WATCHER)) {
         ...
         try {
            ...
            this.getLogger().at(Level.SEVERE).withCause(var4).log("Failed to create asset monitor!");
         }
      }
   ...
   private void loadAndRegisterPack(Path packPath) {
      ...
      try {
         ...
         this.getLogger().at(Level.WARNING).withCause(var7).log("Failed to load manifest for pack at %s", packPath);
         return;
      }
