   ...
   public JWTValidator.JWTClaims validateToken(@Nonnull String accessToken, @Nullable X509Certificate clientCert) {
      if (accessToken.isEmpty()) {
         ...
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(var10).log("Failed to parse JWT");
            return null;
         } catch (Exception var11) {
            LOGGER.at(Level.WARNING).withCause(var11).log("JWT validation error");
            return null;
         }
      }
   }
   ...
   public JWTValidator.IdentityTokenClaims validateIdentityToken(@Nonnull String identityToken) {
      if (identityToken.isEmpty()) {
         ...
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(var8).log("Failed to parse identity token");
            return null;
         } catch (Exception var9) {
            LOGGER.at(Level.WARNING).withCause(var9).log("Identity token validation error");
            return null;
         }
      }
   }
   ...
   public JWTValidator.SessionTokenClaims validateSessionToken(@Nonnull String sessionToken) {
      if (sessionToken.isEmpty()) {
         ...
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(var8).log("Failed to parse session token");
            return null;
         } catch (Exception var9) {
            LOGGER.at(Level.WARNING).withCause(var9).log("Session token validation error");
            return null;
         }
      }
   }
